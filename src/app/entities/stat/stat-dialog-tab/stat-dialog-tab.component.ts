...
export class StatDialogTabComponent {
  ...
  openStatEdit() {
    this.matDialog
      .open(StatsEditComponent, { data: { statValues: this.statValues } })
      .afterClosed()
      .subscribe(async (statValues: StatValue[]) => {
        for (let i = statValues.length - 1; i > -1; i--) {
          ...
          if (statValues[i].id == 0 && statValues[i].value != 0) {
            ...
            (
              ...
              statValues[
                statValues.findIndex((y) => {
                  return y.statId == x.statId;
                })
              ] = x;
            });
          }
          ...
          if (
            statValues[i].id != 0 &&
            (statValues[i].value == 0 || statValues[i].value.toString() == '')
          ) {
            (
            ...
            statValues[i].id != 0 &&
            statValues[i].value !=
              this.statValues.find((x) => x.statId == statValues[i].statId)
                ?.value
          ) {
            (
