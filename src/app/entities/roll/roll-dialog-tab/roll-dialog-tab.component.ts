...
export class RollDialogTabComponent {
  ...
  openRollEdit() {
    this.matDialog.open(RollsEditComponent, { data: { rolls: this.rolls } })
      .afterClosed().subscribe(async (rolls: Roll[]) => { 
        ...
        for(let i = rolls.length-1; i > -1; i--) {
          ...
          if(rolls[i].id == 0) {
            (await this.requestService.create(this.requestService.routes.rolls, rolls[i])).subscribe((x: any) => {
              rolls[i] = x
            })
          }
          else {
            this.rolls.forEach(async x => {
              if(x.id == rolls[i].id && (x.instruction != rolls[i].instruction || x.name != rolls[i].name)) {
                (await this.requestService.patch(this.requestService.routes.rolls, rolls[i].id, JSON.stringify({
                  "name": rolls[i].name,
        ...
        this.rolls.forEach(async origRoll => {
          if(!(rolls.some(newRoll => newRoll.id == origRoll.id))) {
            (await this.requestService.delete(this.requestService.routes.rolls, origRoll.id)).subscribe()
          }
        })
