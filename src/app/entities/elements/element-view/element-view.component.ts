...
export class ElementViewComponent implements OnInit {
  ...
  constructor(protected requestService: RequestService, private matDialog: MatDialog,
    @Inject(MAT_DIALOG_DATA) public data: { id: number, route: string },
    public dialogRef: MatDialogRef<ElementViewComponent>, protected persistencyService: PersistencyService) 
  {
    if(data.route.includes('Counter')) {
      this.parentData = new ParentData(ParentType.Counter, data.route, data.id, false)
    }
    else {
      this.parentData = new ParentData(ParentType.Modifier, data.route, data.id, false)
    }
  }
  ...
  editElement() {
    this.matDialog.open(ElementEditComponent, { maxWidth: '90vw', data: { id: this.element?.id, route: this.data.route }}).afterClosed().subscribe(async x => {
      if(x === false) {
