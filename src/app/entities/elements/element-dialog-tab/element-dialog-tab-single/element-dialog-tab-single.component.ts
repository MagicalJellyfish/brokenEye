...
export class ElementDialogTabSingleComponent {
  ...
  viewElement(id: number) {
    this.matDialog.open(ElementViewComponent, { maxWidth: '90vw', data: { id: id, route: this.elementRoute }}).afterClosed().subscribe(async _ => {
  ...
  async addElement() {
    this.matDialog.open(TemplateSelectComponent, { data: { route: this.requestService.elementToTemplateRoute(this.elementRoute) }}).afterClosed().subscribe(async template => {
      (await this.requestService.get(this.requestService.elementToTemplateRoute(this.elementRoute) + "/Instantiate", template.id)).subscribe(async (newElement: any) => {
        switch(this.parentData.parentType) {
          case ParentType.Modifier:
            newElement.modifierId = this.parentData.parentId
            break;
        ...
        (await this.requestService.create(this.elementRoute, newElement)).subscribe(x => {
          this.element = x
          this.elementTable = new MatTableDataSource([this.element!])
        })
      })
  ...
  async removeElement() {
    this.matDialog.open(ConfirmationDialogComponent, { data: { message: "Are you sure you want to delete this element?" }}).afterClosed().subscribe(async x =>
      {
        if(x) {
          (await this.requestService.delete(this.elementRoute, this.element.id)).subscribe(_ => {
